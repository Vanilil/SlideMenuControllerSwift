...
open class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    open func initView() {
        ...
        if topViewController != nil {
            ...
            topContainerView.autoresizingMask = UIView.AutoresizingMask.flexibleHeight
    ...
    open func handleContainerSizeChange() {
        ...
        if topViewController != nil {
            ...
            view.insertSubview(topContainerView, at: 2)
            setUpViewController(topContainerView, targetViewController: topViewController)
        } else if topLeftViewController != nil {
            topContainerView.removeFromSuperview()
            var topFrame: CGRect = view.bounds
            topFrame.size.height = SlideMenuOptions.topViewHeight
            topFrame.origin.y = topMinOrigin()
            ...
            topContainerView = UIView(frame: topFrame)
            view.insertSubview(topContainerView, at: 2)
            setUpViewController(topContainerView, targetViewController: topLeftViewController)
        } else if topRightViewController != nil {
            topContainerView.removeFromSuperview()
            var topFrame: CGRect = view.bounds
            topFrame.size.height = SlideMenuOptions.topViewHeight
            topFrame.origin.y = topMinOrigin()
            ...
            topContainerView = UIView(frame: topFrame)
            view.insertSubview(topContainerView, at: 2)
            setUpViewController(topContainerView, targetViewController: topRightViewController)
        }
...
extension UIViewController {
    ...
    public func addTopBarButtonWithImage(_ buttonImage: UIImage, _ isLeft: ObjCBool, _ getItems: [UIBarButtonItem]) {
        var topButton: UIBarButtonItem = UIBarButtonItem(image: buttonImage, style: UIBarButtonItem.Style.plain, target: self, action: #selector(self.toggleTop))
        
        if isLeft.boolValue
        {
            topButton = UIBarButtonItem(image: buttonImage, style: UIBarButtonItem.Style.plain, target: self, action: #selector(self.toggleTopLeft))
        }
        else
        {
            topButton = UIBarButtonItem(image: buttonImage, style: UIBarButtonItem.Style.plain, target: self, action: #selector(self.toggleTopRight))
        }
